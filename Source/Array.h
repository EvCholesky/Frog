...
public:
	...
	const T &	operator[](size_t i) const	{ FR_ASSERT((i>=0) & (i<m_c), "array overflow"); return m_a[i]; }
	T &			operator[](size_t i)		{ FR_ASSERT((i>=0) & (i<m_c), "array overflow"); return m_a[i]; }
	...
	T *			PLast()						
					{ 
						if (FR_FVERIFY(m_c > 0, "using PLast on empty Ary"))
							return &m_a[m_c-1];
						return nullptr;
					}
	T 			Last() const			
					{ 
						if (FR_FVERIFY(m_c > 0, "using Last on empty Ary"))
							return m_a[m_c-1];
						return T();
					}
	const T *	PLast() const
	...
	size_t		IFromP(const T * pT) const
					{ 
						...
						FR_ASSERT((iT >= 0) & (iT < m_c), "pointer not contained within array bounds");
						return iT; 
					}
	...
	void		Append(const Type t)
					{
						FR_ASSERT(m_c + 1 <= m_cMax, "fixed array overflow, %d + 1 > %d", m_c, m_cMax);
						T * retValue = &m_a[m_c++];
	...
	void		Append(const Type * pTArray, size_t cT)
					{
						...
						FR_ASSERT(m_c + cT <= m_cMax, "fixed array overflow. %d + %d > %d", m_c, cT, m_cMax);
						T * pTEnd = &m_a[m_c];
	...
	void		AppendFill(size_t c, const Type t)
					{
						FR_ASSERT(m_c + c <= m_cMax, "fixed array overflow");
	...
	T *			AppendNew()
					{
						FR_ASSERT(m_c + 1 <= m_cMax, "fixed array overflow");
						T * retValue = &m_a[m_c++];
						Construct(retValue);
						return retValue;
					}
	...
	void		AppendNew(size_t cNew)
					{
						FR_ASSERT(m_c + cNew <= m_cMax, "fixed array overflow");
						T * retValue = &m_a[m_c];
	...
	void		PopLast()
					{
						if (FR_FVERIFY(m_c > 0, "array underflow"))
						{
							--m_c;
							Destruct(&m_a[m_c]);
						}
...
struct FR_ALIGN(16)	AlignedQword  					{ u8 m_a[16]; };
template <int ALIGNMENT> struct AlignedBlock		{ AlignedBlock() {FR_CASSERT(sizeof(*this), "unknown alignment in SAlignedBlock");}  };
...
public:
	...
	void		Append(const Type t)
					{
						FR_ASSERT(m_c < m_cMax, "FixAry overflow");
						T * retValue = &m_a[m_c++];
						CopyConstruct(retValue, t);
					}
	...
	void		Append(const Type * pTArray, size_t cT)
					{
						...
						FR_ASSERT(m_c + cT < m_cMax, "FixAry overflow");
						T * pTEnd = &m_a[m_c];
						CopyConstructArray(pTEnd, cT, pTArray);
						m_c += cT;
					}
	...
	AlignedBytes<sizeof(T) * C_MAX, FR_ALIGN_OF(T)>	m_alby;
};
...
public:
	...
	void		RemoveFastByI(size_t iT)
					{
						if (FR_FVERIFY((iT >= 0) & (iT < m_c), "bad element index" ))
						{
							--m_c;
							Destruct(&m_a[iT]);
							if (iT != m_c)
	...
	void		Resize(size_t cMax)
					{
						size_t cNewMax = cMax;
						FR_ASSERT(cNewMax == static_cast<s32>(cNewMax), "size overflow in DynAry");
						if (cNewMax == m_cMax)
							return;
						...
						if (cNewMax > 0)
						{
							...
							//m_a = (T *)malloc(cB, FR_ALIGN_OF(T), m_bk);
							//m_a = (T *)std::aligned_alloc(FR_ALIGN_OF(T), cB);
							m_a = (T *)_aligned_malloc(cB, FR_ALIGN_OF(T));
...
struct ByteStream // tag = bstr
{
	...
	void		Swap(ByteStream * pBstr)
					{
						frSwap(m_pB, pBstr->m_pB);
						frSwap(m_cB, pBstr->m_cB);
						frSwap(m_cBMax, pBstr->m_cBMax);
					}
	...
	void		EnsureSize(size_t cB)
					{
						if (cB > m_cBMax) 
						{
							size_t cBNew = frMax(m_cBMax * 2, cB); 
							Resize(cBNew);
						}
					}
